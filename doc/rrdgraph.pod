...
overlaps with the x-axis). heat-base allows the user to specify the 
y-coordinate of the heat base. As a result the whole heat-map is shifted up 
as many units on the y-axis as specified by the user.

[B<--heat-gap> I<value>]

The heat-gap option enables spacing between the separate bars of a heat-map. 
The specified value is the percentage of the heat-bar to be drawn as a gap.
...
If daemon specified, connect-timeout option allows the user to configure 
